use std::{
    ...
    io::{stdin, Read},
    mem::replace,
    ops::{Add, AddAssign, Sub, SubAssign}, collections::{HashMap, hash_map::{Entry, DefaultHasher}}, hash::{Hash, Hasher},
};
...
impl Board {
    ...
    fn commit(&mut self, shape: Shape, left: Point2) {
        for point in shape.outline_for(left) {
...
const SAMPLE: usize = 10000;
const WINDOW: usize = 20;
const ROCK_NO: usize = 1000000000000;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut dirloop = dirs.iter().cloned().enumerate().cycle();

    let mut board = Board::new();

    let mut dbgrid = DebugGrid::new();
    
    let mut pmap : HashMap<_, (Point2, usize)> = HashMap::new();

    let mut heights = vec![];

    let mut cyclemap = HashMap::new();
    let mut cycle = None;

    'probe: for (n, &shape) in SHAPE_PATTERNS.iter().cycle().take(SAMPLE).enumerate() {
        ...
        for (dn, dir) in &mut dirloop {
            ...
            if board.try_insert_shape_at(shape, cur) {
                heights.push(board.top);

                if n >= WINDOW {
                    let mut hsr = DefaultHasher::new();
                    
                    (shape, dn, cur.0).hash(&mut hsr);

                    for (rs, Point2(rx, _)) in &board.rocks[n-WINDOW..n] {
                        (rs, rx).hash(&mut hsr);
                    }
                    
                    use Entry::*;
                    match pmap.entry(hsr.finish()) {
                        Occupied(oe) => {
                            let (oldp, oldn) = *oe.get();

                            match cyclemap.entry((oldp, oldn)) {
                                Occupied(coe) => {
                                    let (diff, hdiff) = *coe.get();

                                    println!("assuming cycle of size {diff}");
                                    
                                    cycle = Some((oldn, diff, hdiff));

                                    break 'probe;
                                },
                                Vacant(cve) => {
                                    let diff = n - oldn;
                                    let hdiff = cur.1 - oldp.1;

                                    cve.insert((diff, hdiff));
                                    
                                    println!("found match: before = {oldn}, cur = {n}, diff = {diff}, hdiff = {hdiff}");
                                }
                            }
        ...
        // let p = (dn, cur, board.top, n);

        // if let &[first, second, third] = &matches[matches.len().saturating_sub(3)..] {
        //     let (n1, n2, n3) = (
        //         first.1 .1 as usize,
        //         second.1 .1 as usize,
        //         third.1 .1 as usize,
        ...
        //     let (delta12, delta23) = (second.2 - first.2, third.2 - second.2);

        //     if board.grid[n1..n2] == board.grid[n2..n3] {
        //         let rep = second.3 - first.3;

        //         println!("cycle detected, of len {delta12}, every #{rep} rocks");

        //         cycle = Some((first.2, rep));
    ...
    if let Some((starts, period, period_height)) = cycle {
        let period = period as usize;

        let rocks = ROCK_NO - 1;
        let full_loops = ((rocks - starts) / period) as isize;
        let rem = (rocks - starts) % period;

        let hrem = heights[starts + rem] - heights[starts];

        let htot = heights[starts] + period_height * full_loops + hrem;

        println!("height at {ROCK_NO} rocks: {htot}");
    } else {
        println!("no cycle found, quit normally. Height: {}", board.top);
    }
