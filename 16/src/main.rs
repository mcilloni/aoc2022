use std::{
    cmp::{max, min},
    // using tree map has 0 performance implications in this case as far as I can tell
...
impl Valves {
    ...
    fn best_flow(&self) -> usize {
        ...
        // we already discovered all solutions through BFS. The only requirement we
        // have is that we need to open different valves than the elephant, i.e. we must find the max among all
        // disjointed solutions

        results
            .iter()
            .cartesian_product(results.iter())
            .filter(|((&b1, _), (&b2, _))| b1 & b2 == 0)
            .map(|((_, r1), (_, r2))| r1 + r2)
            .max()
            .expect("expecting a result")
    }
}
