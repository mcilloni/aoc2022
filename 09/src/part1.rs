...
impl Rope {
    fn move_head(&mut self, dir: Direction) -> Point2 {
        ...
        let head = head + dir.versor();

        let Point2(diff_x, diff_y) = head - tail;

        assert!(diff_x.abs() < 3 && diff_y.abs() < 3); // if this is not true, this code is very broken

        let tail = match (diff_x, diff_y) {
            (x, y) if x.abs() < 2 && y.abs() < 2 => tail, // do nothing, tail is fine
            (x, y) => compute_step(tail, x, y),           // fixup
        };
