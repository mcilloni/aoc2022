...
fn blanks_in_pointset(points: &HashSet<Point2>) -> usize {
...
fn do_round(mut points: HashSet<Point2>, sides: &[Direction]) -> HashSet<Point2> {
    use Slot::*;
...
fn do_rounds(mut points: HashSet<Point2>, rounds: usize) -> HashSet<Point2> {
    use Direction::*;

    println!("Initial:");
    dump_pointset(&points);

    let mut directions = [North, South, West, East];

    for r in 0..rounds {
        points = do_round(points, &directions);

        println!("Round #{} <{:?}>", r + 1, directions[0]);
        dump_pointset(&points);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let points = do_rounds(read_points(file)?, ROUNDS);

    println!("blanks = {}", blanks_in_pointset(&points));
